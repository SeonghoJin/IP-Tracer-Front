...
export const RawDataView = () => {
    ...
    const onKeyDown : KeyboardEventHandler = useCallback((e) => {
        ...
        if(command === 'IPTracer clear' || command === 'clear'){
            ...
            setRawMessages((prev) => prev.concat([
        ...
        } else if(parsedCommand[0] === 'IPTracer' && parsedCommand[1] === 'start' && parsedCommand[2] != null) {
    ...
    <CommandInput ref={scrollRef} value={command} type={"text"} onChange={onChangeCommand} onKeyDown={onKeyDown}/>
    </div>
}
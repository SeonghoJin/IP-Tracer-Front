...
describe('lookup-ip-api', () => {
  ...
  beforeEach(async () => {
    ...
    api = moduleRef.get(LookupStackApi);
  ...
  describe('lookup()', function () {
    ...
    it('api이용해서 lookup하고 status가 USAGE_LIMIT_STATUS이면, BadRequest에러를 던진다.', async () => {
      ...
      const result: AxiosResponse<IpLocationResponseDto> = {
        config: undefined,
        headers: undefined,
        status: IpStackError.USAGE_LIMIT_REACHED_STATUS,
        statusText: 'error',
        data,
      };
      ...
      try {
        ...
        expect(e).toStrictEqual(
          new BadRequestException(
            IpStackError[IpStackError.USAGE_LIMIT_REACHED_STATUS],
          ),
        );
      }
    });
    ...
    it('api이용해서 lookup하고 status가 USAGE_LIMIT_REACHED_STATUS 라면, Status를 USAGE_EXCEED로 변경한 후 BadRequestException을 던진다.', async () => {
      const data = new IpLocationResponseDto();
      const result: AxiosResponse<IpLocationResponseDto> = {
        config: undefined,
        headers: undefined,
      ...
      jest
        ...
        .mockImplementationOnce(
          async (
            ...
            if (apiName === LookupApiName.stackApi) {
